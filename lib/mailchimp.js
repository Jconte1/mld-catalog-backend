...
async function syncOne(config, attendee, meta = {}) {
  ...
  const emailStatus = attendee?.emailConsent
    ? (config.doubleOptIn ? "pending" : "subscribed")
    : "transactional";
...
async function syncSmsConsent(config, attendee) {
  ...
  const smsConsentStatus = "confirmed";
...
export async function syncSaleSignupsToMailchimp(attendees = [], meta = {}) {
  ...
  if (!config.enabled) {
    return {
      ...
      successCount: 0,
      failedCount: 0,
      skippedCount: attendees.length,
      skippedAttendees: attendees.map((a) => ({
        email: a?.email || null,
        reason: "Mailchimp disabled.",
      })),
      failures: [],
      warnings: [],
    };
  ...
  const eligibleAttendees = attendees.filter(
    (attendee) => attendee?.emailConsent || attendee?.smsConsent
  );
  const skippedAttendees = attendees
    .filter((attendee) => !attendee?.emailConsent && !attendee?.smsConsent)
    .map((attendee) => ({
      email: attendee?.email || null,
      reason: "No marketing consent for email or SMS.",
    }));
  ...
  return {
    ...
    skippedCount: skippedAttendees.length,
    skippedAttendees,
    failures,
    warnings,
  };
